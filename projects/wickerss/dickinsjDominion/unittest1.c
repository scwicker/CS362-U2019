...
int main() {
 ...
 G.deckCount[1] = 1;
 callMine( 1, 1, 1, 2, 1, &G, bonus);
 if (G.deckCount[1] == 1) {
     printf("FAIL: deck count should not have changed.\n");
 }
 else{
     printf("pass : deck count stayed same  \n");
 }
    ...
    if(callMine(1, 1, 2, 1, 2, &G, bonus) == 0){
        ...
        printf("Failed: callMine card wasn't validated properly\n");
    }
    ...
    if(callMine(1, 1, 2, 1, 2, &G, bonus) == 0){
        ...
        printf("Failed: callMine allowed card out of range curse\n");
    }
